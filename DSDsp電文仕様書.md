...
### 2.4 ヒート状態変化通知（進行タブ自動遷移）

#### MC_HEAT_NOTIFY（サーバー → クライアント）

**目的**: DSMusic（MU）または DSMc（MC）からのヒート終了を受けたサーバーが、同一競技会に接続中のすべてのクライアント（GM・MC・MU・DSDsp）へブロードキャストする通知。
DSDsp は `イベント = "END"` のときのみ進行タブの自動遷移処理を実行する。

**電文例**:

```
JS,123456,SVR,MC_HEAT_NOTIFY,{"フロア記号":"A","進行番号":"5","種目順":2,"ヒート番号":3,"イベント":"END",...}
```
...
```json
{
  ...
  "ヒートステータス": "5",
  "進行ステータス": "4",
  "開始実績時刻": "2026-06-20T11:30:00.000+09:00",
  "終了実績時刻": "2026-06-20T11:31:30.000+09:00"
}
```
...
| フィールド | 型 | 説明 |
|---|---|---|
| フロア記号 | string | フロアコード（例: "A"） |
| 進行番号 | string | 終了したヒートが属する進行番号 |
| 進行番号枝番 | string | 進行番号枝番（通常 "0"） |
| 種目順 | int | 種目番号（DS_DncNo） |
| ヒート番号 | int | ヒート番号 |
| イベント | string | "START" / "END" / "CANCEL" のいずれか |

**DSDsp での処理（イベント = "END" のとき）**:

1. `進行番号` と `フロア記号` から DS_Status の `KbnNo` / `RndNo` を特定する。
2. 進行一覧（`LstProgressItems`）から対応する `ProgressListItem` を検索する。
3. 現在表示中の進行と異なる場合は、電文の進行にカーソルを移動（**電文内容を優先**）。
4. 同一進行内に次ヒートが存在する場合は `Advance()` を呼び出してヒート表示を更新する。
5. 全ヒート・全種目が終了している場合は、次の進行の STEP1 から表示を開始する。
6. `⟳ 自動更新` トグルが OFF の場合は何もしない。
...
| MC_HEAT_NOTIFY   | S→C  | ヒート状態変化通知（イベント="END" のとき自動遷移トリガー） |
