...
namespace DSDsp.画面
{
    ...
    /// </summary>
    public partial class DSP_GRP_001_S_出場選手一覧_小 : DSDspScreenBase
    {
        #region 定数定義
        ...
        public void Step1()
        {
            ...
            _ページ数 = Math.Max(1, (int)Math.Ceiling(_総表示件数 / 10.0));
        ...
        public void Step3(JsonNode? dvResult, int 開始インデックス = 0)
        {
            ...
            var 表示対象 = 表示データリスト.Skip(開始インデックス).Take(10).ToList();
            int 表示件数 = 表示対象.Count;
            if (表示件数 == 0) return;
