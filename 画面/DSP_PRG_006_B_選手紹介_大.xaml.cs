...
{
    /// <summary>
    /// DSP_PRG_006_B_選手紹介_大.xaml の相互作用ロジック
    ///
    /// ステップ構成:
    ...
    /// </summary>
    public partial class DSP_PRG_006_B_選手紹介_大 : DSDspScreenBase
    {
        #region 定数定義
        private const double SLIDE_FROM_LEFT         = -2000;
        private const double SLIDE_FROM_RIGHT        = 2000;
        private const int    FADE_DELAY_MILLISECONDS = 800;
        private const string FONT_FAMILY_NAME        = "Segoe UI Semibold";

        // フォントサイズ調整定数（大画面用 LB 幅 800px に合わせる）
        private const double MAX_WIDTH = 800;
        private const double MAX_FS    = 28;
        private const double MIN_FS    = 10;
        #endregion
        ...
        public void Step2()
        {
            ...
            if (HonorMode)
            {
                ...
                _partsMain.フォントサイズ自動調整(PartsPRG006.LB_区分名,  PartsPRG006.LB_区分名.Content?.ToString() ?? "", MAX_WIDTH, 22, MIN_FS, FONT_FAMILY_NAME);
            ...
            else
            {
                ...
                _partsMain.フォントサイズ自動調整(PartsPRG006.LB_区分名,  lb区分テキスト,     MAX_WIDTH, 22,      MIN_FS, FONT_FAMILY_NAME);
