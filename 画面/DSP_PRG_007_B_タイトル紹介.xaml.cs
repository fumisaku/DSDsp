...
namespace DSDsp.画面
{
    ...
    /// </summary>
    public partial class DSP_PRG_007_B_タイトル紹介 : DSDspScreenBase
    {
        #region 定数定義
        ...
        #region プロパティ
        /// <summary>
        /// LB_Title1 に表示するテキストを直接指定する場合に設定する。
        /// null または空文字の場合は DA_Master から区分名を取得する通常動作。
        /// </summary>
        public string? Title1Override { get; set; } = null;

        /// <summary>
        /// LB_Title2 に表示するテキストを直接指定する場合に設定する。
        /// null または空文字の場合は DA_Master からラウンド名を取得する通常動作。
        /// </summary>
        public string? Title2Override { get; set; } = null;
        #endregion

        #region コンストラクタ
        public DSP_PRG_007_B_タイトル紹介()
        {
            InitializeComponent();
        }
        #endregion
        ...
        public void Step2()
        {
            ...
            // 表示テキストを決定（Overrideが設定されていればそちらを優先）
            string title1Text = !string.IsNullOrEmpty(Title1Override)
                ? Title1Override!
                : DSDspDataHelper.Get区分名(DA_Master, 区分番号);
            string title2Text = Title2Override != null
                ? Title2Override
                : DSDspDataHelper.Getラウンド名(DA_Master, 区分番号, ラウンド番号);

            // COM001 TB_左上2: Overrideがある場合は空文字、通常は区分+ラウンド名
            if (!string.IsNullOrEmpty(Title1Override))
                PartsCOM001.TB_左上2.Text = string.Empty;
            else
                PartsCOM001.TB_左上2.Text = title1Text + "　" + title2Text;
            ...
            // LB_Title2: title2Textが空の場合は非表示
            PartsTIT001.LB_Title2.Visibility = string.IsNullOrEmpty(title2Text)
            ...
            if (!string.IsNullOrEmpty(title2Text))
                _partsMain.フォントサイズ自動調整(
